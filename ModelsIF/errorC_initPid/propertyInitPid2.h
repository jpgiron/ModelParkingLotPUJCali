...
inline void if_t_sTemp_reset
    (if_t_sTemp_type& x);
#define if_t_sTemp_iterate(x)\
  if_pid_iterate(x.p1)
inline if_t_sTemp_type if_t_sTemp_make
    (if_pid_type p1);
#define if_t_sTemp_eq(x,y) if_t_sTemp_compare(x,y)==0
#define if_t_sTemp_ne(x,y) if_t_sTemp_compare(x,y)!=0
...
/* 
 * sTemp message interface 
 *
 */

#define if_sTemp_signal 12
...
inline void if_sTemp_par_reset
    (if_sTemp_par_type& x);
#define if_sTemp_par_iterate(x)\
  if_pid_iterate(x.p1)
inline if_sTemp_par_type if_sTemp_par_make
    (if_pid_type p1);
#define if_sTemp_par_eq(x,y) if_sTemp_par_compare(x,y)==0
#define if_sTemp_par_ne(x,y) if_sTemp_par_compare(x,y)!=0

class if_sTemp_message : public IfMessage {

public:
  if_sTemp_message();
  if_sTemp_message(if_pid_type p1);
  if_sTemp_message(const if_sTemp_message&);
...
#define if_sOkInitPid_signal 13
...
#define if_sInitPidCtrl_signal 14
...
//         void tpc();
  ...
  inline void _RTDS_START_1f_fire();


         void _sWait_dispatch(IfMessage*);

         void _sWait_1_fire(IfMessage*);
  inline void _sWait_1a_fire();
  inline void _sWait_1b_fire();
...
//  typedef void (if_pZone_instance::*tpcchecker)();
  static if_state<16,dispatcher/*,tpcchecker*/> STATE[];
...
private:
  ...
  static if_state<16,dispatcher> STATE[];
